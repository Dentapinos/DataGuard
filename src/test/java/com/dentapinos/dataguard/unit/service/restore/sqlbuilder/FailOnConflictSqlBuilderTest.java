...
import com.dentapinos.dataguard.service.restore.sqlbuilder.FailOnConflictSqlBuilder;
...
/**
 * Юнит-тесты для FailOnConflictSqlBuilder.
 * Проверяет генерацию SQL INSERT-запросов для нескольких колонок, обработку пустых списков
 * и экранирование имен таблиц и колонок обратными апострофами.
 */
@DisplayName("Unit-test для генератора SQL-запросов с обработкой конфликтов")
@SpringBootTest
class FailOnConflictSqlBuilderTest {

    @Autowired
    private FailOnConflictSqlBuilder builder;

    @Test
    @DisplayName("должен корректно генерировать INSERT для нескольких колонок")
    ...
    @Test
    @DisplayName("должен корректно генерировать INSERT для одной колонки")
    ...
    void shouldHandleEmptyColumnsList() {
        ...
        // assert
        assertEquals("INSERT INTO `logs` () VALUES ()", sql);
        // Примечание: SQL синтаксически валиден, но не имеет смысла в большинстве СУБД.
        // При необходимости можно расширить логику с проверкой: выбрасывать исключение при пустом списке
    }
